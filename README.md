...
## 実行方法 (ローカル環境)
1. MySQLを起動し、データベース「qrkanri」を作成
2. application.propertiesにDB接続情報を設定
3. プロジェクトをビルドし、アプリケーションを起動
4. ブラウザでlocalhostにアクセス
* DBテーブルはJPAにより自動生成されます。
* QRコード読み取りの際、カメラアクセスのためngrokを使用したHTTPS環境が必要になる場合がございます。
